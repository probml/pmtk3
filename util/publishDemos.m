...
fid = setupHTMLfile(fullfile(folder, 'index.html'));
...
for i=1:numel(sortedInfo) 
    ...
    lprintf(sortedInfo(i).localLink, sortedInfo(i).name);
    lprintf(sortedInfo(i).googleLink, sortedInfo(i).description);
    fprintf(fid,'</tr>\n');
end
fprintf(fid,'</table>');
closeHTMLfile(fid);
...
fprintf(fid,'<html>\n');
fprintf(fid,'<head>\n');
fprintf(fid,'<font align="left" style="color:#990000"><h2>PMTK3 Demos</h2></font>\n');
