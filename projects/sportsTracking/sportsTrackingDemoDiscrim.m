...
if 0
% visualize colors assigned to each kind of fvec
figure; hold on
c = 1;
for p=1:Nplayers
   color  = cancolors{p}{1};
   plot(c,0,'o','markerfacecolor',color,'markersize',10);
   c=c+1;
end
verticalLine(c-1);
p=Nplayers+1;
for m=1:numel(cancolors{p})
   color  = cancolors{p}{m};
   plot(c,0,'o','markerfacecolor',color,'markersize',10);
   c=c+1;
end
title('frontal players to left of line, side views to right') 
print(gcf, '-dpng', fullfile(folder, 'sportsPlayerColors.png'))
end
...
str = sprintf('train iid hard %d. Test KL %5.3f, errrate %5.3f', Ntrain, loss, errRate)
set(gcf, 'name', str);
...
drawBel(bel, dataTest);
[loss, errRate] =  evalPerf(belTestHard, dataTest);
hardLossIID(sizeTrial, engineTrial) = loss;
hardErrIID(sizeTrial, engineTrial) = errRate;
str = sprintf('train iid hard %d. Test iid KL %5.3f, errrate %5.3f', Ntrain, loss, errRate)
set(gcf, 'name', str);
...
str = sprintf('train soft %d. Test KL %5.3f, errrate %5.3f', Ntrain, loss, errRate)
set(gcf, 'name', str);
else
  softLoss = [];
end
...
%figure;
subplot(nr,nc,3)
plot(trainSizes, softLoss, 'o-');
xlabel('training size');
title('KL soft')

%figure;
subplot(nr,nc,4)
plot(trainSizes, softErr, 'o-');
xlabel('training size');
title('err soft')
end
