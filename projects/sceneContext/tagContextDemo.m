...
%% Models/ methods
methodNames  = { 'mix1lev', 'mix1G', 'mix5G', 'treeG'};
%methodNames  = { 'tree'};
...
% noisyMixModel with K=1 and localev is equivalent
% to just using independent classifiers since
% we just multiply local likelihood by a fixed prior
% that is independent of the data.
% Using gauss instead of localev loses some performance.
% Using localev with K=5 also gives no change over indep...

fitMethods = {
  @(truth, scores) noisyMixModelFit(truth, scores, 1, 'localev'), ...
  @(truth, scores) noisyMixModelFit(truth, scores, 1, 'gauss'), ...
  @(truth, scores) noisyMixModelFit(truth, scores, 5, 'gauss'), ...
...
fitMethods = {
  @(truth, scores) obsModelFit(truth, scores)
  };
...
infMethods = {
  @(model, localev) argout(2, @noisyMixModelInferNodes, model, localev), ...
  @(model, localev) argout(2, @noisyMixModelInferNodes, model, localev)
  };


infMethods = {
  @(model, localev) obsModelInferNodes(model, localev)
  };


infMethods = {
  @(model, localev) argout(2, @treegmInferNodes, model, localev)
  };
