function [model, lambdaStar, kernelParamStar, mu, se] = linregKernelFitL1CV...
    (X, y, kernelParamRange, kernelType, lambdaRange, nfolds)
% L1 Linear Regression with a kernel basis expansion of the data, and
...
if nargin < 5 || isempty(lambdaRange)
    Xc = mkUnitVariance(center(X));
    K = kernelBasis(Xc, Xc, kernelType, mean(kernelParamRange));
    lambdaRange = linspace(0, lassoMaxLambda(K, center(y)), 15)';
end
if nargin < 6, nfolds = 5; end

fitFn = @(X, y, param)linregKernelFitL1(X, y, param(1), param(2), kernelType);
