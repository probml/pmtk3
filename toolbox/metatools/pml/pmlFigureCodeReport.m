...
fullDest = fullfile(dest, ['pml', modDate]);
...
for i=1:nfigs
    ...
    if ~isempty(codeNames)
        for j=1:numel(codeNames)
            ...
            if isempty(gcl)
                sourceLink = sprintf('%s%s<br>', sourceLink, codeNames{j});
            else
...
header = [...
    sprintf('<font align="left" style="color:%s"><h2>Figures from "Machine Learning: a Probabilistic Approach"</h2></font>\n', pmtkRed),...
    sprintf('<br>Auto-generated by %s.m on %s from book version of %s<br>\n', ...
    mfilename(), date(), modDate),...
    sprintf('<br>Image files are in pdf format.'), ...
    sprintf('<br>Most source files are matlab and are stored at pmtk.googlecode.com (just click on the filename to see the source).\n'),...
    sprintf('<br>Source files ending in .ppt are powerpoint.\n'),...
     sprintf('<br>Source files ending in .tex are latex.\n'),...
     sprintf('<br>Images without source files are hand-drawn or taken from other sources.\n'),...
     sprintf('<br>\n'),...
    sprintf('<br> Total number of figures: %d', numel(Finfo)), ...
    sprintf('<br>\n')...
    ];
