...
header = [...
    sprintf('<font align="left" style="color:%s"><h2>PML Figures</h2></font>\n', pmtkRed),...
    sprintf('<br>Revision Date: %s<br>\n', modDate),...
    sprintf('<br>PML version: %s<br>\n', modDate), ...
    sprintf('<br>Auto-generated by %s.m<br>\n', mfilename()),...
    sprintf('<br>Most source files are matlab and are stored at pmtk.googlecode.com (just click on the filename to see the source).<br>\n'),...
    sprintf('<br>Source files ending in .ppt are powerpoint.<br>\n'),...
     sprintf('<br>Source files ending in .tex are latex and need pstricks.<br>\n'),...
     sprintf('<br>Images without source files are hand-drawn or taken from other sources.<br>\n'),...
