...
% The multiline comment below is parsed and used as the header text.
header = formatHtmlText(pmtkRed, mfilename, date, modDate, numel(Finfo));  
%{
...
Most source files are matlab and are stored at pmtk.googlecode.com (just click on the filename to see the source).
Source files ending in .ppt are powerpoint.
Source files ending in .tex are latex.
