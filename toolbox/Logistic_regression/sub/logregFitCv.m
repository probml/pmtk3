function [model, lambdas, muLoss, seLoss] = logregFitCv(X, y, varargin)
...
if isempty(fitFn)
    switch lower(regType)
        case 'l1'
          %L1GeneralProjection(gradFunc,w,lambda,params,varargin)
        ...
        case 'l2'
          ...
          %w = minFunc(penobjective, winit(:), opts);
           ...
           %(objective, winit, l, opts, X, y);
           %fitFn = @(obj, w, lambda) minFunc( penalizedL2(w, obj, lambda, X, y), winit(:), opts);
    end
end
opts = fitOptions;
if isempty(opts)
