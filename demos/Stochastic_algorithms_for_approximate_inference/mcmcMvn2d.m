...
for j=1:length(S)
    ...
    title(ttl)
    printPmtkFigure(sprintf('gauss2d%sSamples', strrep(strrep(names{j},' ',''),'.','')));
    ...
    for i=1:2
        ...
        xs = linspace(-4, 4, 100)'; 
        plot(xs, scale*exp(gaussLogprob(margExact{i}, xs)), 'linewidth', 2', 'color', 'r');
        title(sprintf('exact m=%5.3f, v=%5.3f', margExact{i}.mu, margExact{i}.Sigma));
        subplot2(2, 2, i, 2);
        if statsToolboxInstalled()
            ksdensity(samples(:, i)); 
            title(sprintf('approx m=%5.3f, v=%5.3f', mean(samples(:, i)), var(samples(:, i))));
        end
    end
    suptitle(ttl);
    printPmtkFigure(sprintf('gauss2d%sMarginals', strrep(strrep(names{j},' ',''),'.','')));
