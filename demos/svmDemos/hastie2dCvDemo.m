...
% and gamma in an svm to show their interdependence. We also show the 2D
% surface similar to figure 7 in
% http://www.csie.ntu.edu.tw/~cjlin/papers/guide/guide.pdf.
%PMTKslow
%% Load Data
setSeed(0);
data = loadHastieMixtureData();
%X = mkUnitVariance(center(data.X)); 
X = data.X;
y = convertLabelsToPM1(data.y); 
[X, y] = shuffleRows(X, y); % data is ordered
%% Plot 1D CV Slices
gammas = [5, 1, 0.5, 0.1];
...
[model, bestParams, CVmu, CVse] = svmlightFitCV(X, y, 'kernelParam', gammas, 'C', C, 'cvopts', {5, false, true});
xlabel('C');
ylabel('\gamma');
set(gca, 'xscale', 'log', 'yscale', 'log'); 
title({sprintf('best C = %.2f', bestParams(1));['best \gamma', sprintf(' = %.2f', bestParams(2) )]});
%% Plot 2D contour
figure
nc = numel(C);
ng = numel(gammas); 
