...
[junk, yhatBayes] = max(logprobBayes + logprior,[],2); % max across classes

logprobPlugin = sum(logprobPluginC,3); % sum across features
logprior = repmat(log(mle.pi),Ntest,1);
[junk, yhatPlugin] = max(logprobPlugin + logprior,[],2); % max across classes
