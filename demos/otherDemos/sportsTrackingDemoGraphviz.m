% PMTKinprogress
function sportsTrackingDemoGraphviz()
...
Nframes = 10;
data = generateData(Nframes, Nplayers, obsModel, cancolors, ...
...
Nnodes = size(G,1);
nodeNames = cell(1,Nnodes);
for t=1:Nframes
  for d=1:data.numPresent(t)
    n = data.node2dTo1d(t,d);
    nodeNames{n} = sprintf('%d,%d', t, d);
    nodeColors(n,:) = colors{data.truth(n)};
  end
end 
...
for t=1:Nframes
  for d=1:data.numPresent(t)
    src = data.node2dTo1d(t,d);
    % temporal edges
    if ~isempty(data.succ{t}) & ~isnan(data.succ{t}(d))
      dest = data.node2dTo1d(t+1, data.succ{t}(d));
      edgeColors{e} = [nodeNames{src}, nodeNames{dest}, colors{1}];
      e=e+1;
    end
    % mutex edges
    for dd=1:data.numPresent(t)
      if dd==d, continue; end
      dest = data.node2dTo1d(t,dd);
      %edgeColors{e} = [nodeNames{src}, nodeNames{dest}, colors{2}];
      edgeColors{e,1} = nodeNames{src};
      edgeColors{e,2} = nodeNames{dest};
...
s = graphViz4Matlab('-adjMat', G, '-nodeLabels', nodeNames, ...
  '-nodeColors', nodeColors, '-undirected', true, ...
...
[bel] = inferBel([], dataTest, opts, 'soft');
drawBel(bel, dataTest);
[loss, errRate] =  evalPerf(bel, dataTest);
lossSoftOracle = loss;
errSoftOracle = errRate;
str = sprintf('no appearance, soft labels. %s, KL %5.3f, err %5.3f', infEngine, loss, errRate);
set(gcf, 'name', str)
suptitle(str)
print(gcf, '-dpng', fullfile(folder, sprintf('sportsBelSoftOracle%s.png', infEngine)));
end
...
str = sprintf('%s, hard N=%d. KL %5.3f, err %5.3f', ...
  infEngine, Ntrain, loss, errRate)
set(gcf, 'name', str);
suptitle(str)
print(gcf, '-dpng', fullfile(folder, sprintf('sportsBelHard%d%s.png',Ntrain,infEngine)))
end
...
str = sprintf('%s, soft N=%d. KL %5.3f, err %5.3f, traintime %5.3f, testtime %5.3f', ...
  infEngine, Ntrain, loss, errRate, trainTime, testTime)
set(gcf, 'name', str);
suptitle(str)
print(gcf, '-dpng', fullfile(folder, sprintf('sportsBelSoft%d%s.png',Ntrain, infEngine)))
else
  softLoss = [];
end
...
figure; hold on
X = [rowvec(hardLoss); rowvec(softLoss); rowvec([lossSoftOracle lossSoftOracle])];
% columns are jtree, BP
% rows are hard, soft, softOracle
bar(X);
title('KL')
set(gca, 'xtick',1:3,'xticklabel', {'hard', 'soft', 'softOracle'})
legend('jtree', 'BP')
print(gcf, '-dpng', fullfile(folder, sprintf('sportsKL.png')))

figure; hold on
X = [rowvec(hardErr); rowvec(softErr); rowvec([errSoftOracle errSoftOracle])];
% columns are jtree, BP
% rows are hard, soft, softOracle
bar(X);
title('err')
