...
function results = classificationShootoutCvLambdaOnlyLinear()
setSeed(0);
doLatex = true; 
doHtml  = true;
...
dataSets(d).y = y;
dataSets(d).name = 'Soy';
dataSets(d).nClasses = 3;
dataSets(d).nFeatures = 35; 
%dataSets(d).kernel = 'rbf';
dataSets(d).kernel = 'linear';
d=d+1;
...
dataSets(d).X = X;
dataSets(d).y = y;
dataSets(d).name = 'colon (linear)';
dataSets(d).nClasses = 2;
dataSets(d).nFeatures = 2000;
dataSets(d).kernel = 'linear';
d=d+1;
end
